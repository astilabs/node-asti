...
//
// ```
// config.statsd = statsd; // node-yodlr statsd object(required)
// config.logger = logger; // node-yodlr logger object(optional)
// config.interval = 2000; // defaults to 2000ms(optional)
// ```
//
var Monitor = function Monitor(config) {
  if(!config) {
    throw new YodlrError('ConfigNotProvided', 'node-yodlr.health');
  }
  if(!config.statsd) {
    throw new YodlrError('ConfigStatsdNotProvided', 'node-yodlr.health');
  }
